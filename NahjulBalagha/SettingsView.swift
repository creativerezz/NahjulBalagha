...
struct SettingsView: View {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    ForEach(AIProvider.allCases) { provider in
                        AIProviderRow(
                            ...
                            availability: getProviderAvailability(provider),
                            onSelect: { 
    ...
    private func getProviderAvailability(_ provider: AIProvider) -> String {
        switch provider {
        case .foundationModels:
            #if canImport(FoundationModels)
            return "Available on iOS 18.1+ with Apple Intelligence enabled"
            #else
