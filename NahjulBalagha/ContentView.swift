...
private struct HomeScreen: View {
    ...
    var body: some View {
        ...
        // Sheets for search results and quick-open sections
    ...
    private func handleUserMessage(_ text: String) {
        ...
        // Local quick intents still run (theme/open/search). We'll run them after streaming if not handled.
        let lower = trimmed.lowercased()

        Task { @MainActor in
            // Configure tools so the model can trigger actions directly
            ai.makeToolEnabledSession(openHandler: { (section: AppSection) in
                switch section {
            ...
            if ai.isAvailable {
                ...
                do {
                    ...
                    // Finalize: if we got structured search results, show a sheet
                    if let results = lastPartial.searchResults, !results.isEmpty {
                        searchResults = Array(results.prefix(20))
                        isShowingSearchSheet = true
                    }
                    // If there is an action with setDarkMode, it was already handled by tool; if the model provided action data without calling a tool, handle here as fallback
                    ...
                    // On error, remove the placeholder and fall back
                    if let index = messages.firstIndex(where: { $0.id == placeholderID }) {
                ...
                // AI not available, remove placeholder and fallback
                if let index = messages.firstIndex(where: { $0.id == placeholderID }) {
                    messages.remove(at: index)
                }
                fallbackLocalHandling(lower: lower, original: trimmed)
            ...
            // If the model didn't produce any reply text, provide a minimal confirmation
    ...
    // Local fallback handler extracted from previous logic
    private func fallbackLocalHandling(lower: String, original: String) {
        ...
        // 3) Search intent — default behavior
