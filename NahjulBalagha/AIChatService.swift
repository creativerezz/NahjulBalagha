...
struct AssistantTurn {
    struct Action: Equatable {
        ...
        init(command: String, bool: Bool? = nil) {
            self.command = command
            self.bool = bool
        }
    }

    struct PartialTurn: Equatable {
        var reply: String?
        var searchResults: [String]?
        var action: Action?

        init(reply: String? = nil, searchResults: [String]? = nil, action: Action? = nil) {
            self.reply = reply
...
final class AIChatService: ObservableObject {
    ...
    init() {
        ...
        currentProvider = AIProvider(rawValue: savedProvider) ?? .foundationModels

        selectedModel = UserDefaults.standard.string(forKey: "selected_model") ?? OpenRouterConfig.availableModels[0]
    ...
    func updateAvailability() {
        ...
        case .openRouter:
            availability = OpenRouterConfig.apiKey.isEmpty ? .unavailable(reason: "OpenRouter API key required. Set it in Settings.") : .available
        ...
        AsyncThrowingStream { continuation in
            Task { [weak self] in
                ...
                do {
                    ...
                    - When the user asks to open Sermons, Letters, or Sayings, set action.command to openSermons/openLetters/openSayings.
        ...
        AsyncThrowingStream { continuation in
            Task { [weak self] in
                ...
                do {
                    ...
                    Action commands:
                    - "openSermons" / "openLetters" / "openSayings"
                    - "setDarkMode" with bool true/false
                    """
